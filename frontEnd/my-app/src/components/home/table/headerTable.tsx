...
const HeaderTable = ({ title, totalData } : headerTableProps) => {
  const handleTotalData = () => {
    const Message = () => (<div>MoAnnotation App has <span className="font-extrabold">{totalData}</span>  Annotateur </div>) 
  ...
  return (
    <div className="flex items-center w-full border-b-2 pb-6 border-[#232E3E]">
      ...
      <div className="ml-auto">
        <button className="bg-[#00C97F] hover:hover:brightness-110 text-white font-semibold py-3 px-6 text-sm rounded-md" onClick={handleTotalData}>Total Of Annotateur</button>
      </div>
      <ToastContainer />
    </div>
  )
